    ...
    public ArrayList<Point2D> getRoad(Point2D exit, Point2D start, int[][] map) {
        ...
        if (map[exit.x][exit.y] != 0 && map[exit.x][exit.y] != -1) {
            ...
            while (map[current_position.x][current_position.y] != map[start.x][start.y]) {
                ...
                if (map[current_position.x - 1][current_position.y] == map[current_position.x][current_position.y]
                        - 1) {
                    current_position.x = current_position.x - 1;
                    road.add(new Point2D(current_position.x, current_position.y));
                }

                else if (map[current_position.x][current_position.y - 1] == map[current_position.x][current_position.y]
                        - 1) {
                    current_position.y = current_position.y - 1;
                    road.add(new Point2D(current_position.x, current_position.y));
                }

                else if (map[current_position.x + 1][current_position.y] == map[current_position.x][current_position.y]
                        - 1) {
                    current_position.x = current_position.x + 1;
                    road.add(new Point2D(current_position.x, current_position.y));
                }
